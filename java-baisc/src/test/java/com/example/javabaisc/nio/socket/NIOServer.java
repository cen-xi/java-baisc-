...
//        }
    ...
    private static void handleRequest(SelectionKey key)
        ...
        try {
            ...
            if (key.isAcceptable()) {
                //--2
                System.out.println("判断事件是否就绪==="+System.currentTimeMillis());
                Thread.sleep(1);
            ...
            // 如果说这个key是readable，是个发送了数据过来的话，此时需要读取客户端发送过来的数据
                ...
                //------------6---------------10
                System.out.println("接收到客户端端给我发的消息==="+System.currentTimeMillis());
                Thread.sleep(1);
                ...
                if (count > 0) {
                    //  准备读取刚写入的数据，就是将limit设置为当前position，将position
                    //  设置为0，丢弃mark。一般就是先写入数据，接着准备从0开始读这段数据，就可以用flip
                    //position = 0，limit = 21，仅仅读取buffer中，0 ~21 这段刚刚写入进去的数据
                    buffer.flip();
                ...
                //---------------7
                System.out.println("接收到客户端通知，告诉我可以给他写消息==="+System.currentTimeMillis());
                Thread.sleep(1);
                ByteBuffer buffer = ByteBuffer.allocate(1024);
